import gsap from "gsap";
import { useGSAP } from "@gsap/react";
import * as THREE from "three";
import { useEffect, useLayoutEffect, useRef, useState } from "react";
import { Canvas } from "@react-three/fiber";
import CameraFocusController from "../../../components/CameraFocusController/CameraFocusController";
import FloatBubble from "./FloatBubble";
// import FloatBubble from "./FloatBubble";
// import FloatBubble from "../../../components/FloatBubble/FloatBubble";
// import FeatherDragDrop from "../../../../test/FeatherDrag";

gsap.registerPlugin(useGSAP);

const BubbleFeather_Interaction = ({ masterTl }: any) => {
  const containerRef = useRef<HTMLDivElement>(null);
  const [_focused, setFocused] = useState(false);
  const [_activeId, setActiveId] = useState<string | null>(null);
  const [focusTarget, setFocusTarget] = useState<THREE.Vector3 | null>(null);
  const [resetSignal, setResetSignal] = useState(0);
  // const [dropStarted, setDropStarted] = useState(false);

  const bubbles = [
    { id: 1, position: [-12, 0, -14], delay: 0, variant: 1 },
    { id: 2, position: [0, 2, -10], delay: 1.2, variant: 2 },
    { id: 3, position: [11, -2, -10], delay: 0.6, variant: 3 }, // center hero
    { id: 4, position: [-6, -8, -9], delay: 0.4, variant: 4 },
    { id: 5, position: [4, -10, -12], delay: 0.2, variant: 5 },
  ];

  // useLayoutEffect(() => {
  //   if (!masterTl.current || !containerRef.current) return;

  //   const bubbleTl = gsap.timeline({
  //     onComplete: () => {
  //       // 🔥 trigger physics AFTER entrance animation
  //       setDropStarted(true);
  //     },
  //   });

  //   bubbleTl.from(containerRef.current, {
  //     autoAlpha: 0,
  //     duration: 0.4,
  //     ease: "power2.out",
  //   });

  //   bubbleTl.from(
  //     ".float-bubble",
  //     {
  //       y: 80,
  //       scale: 0.8,
  //       opacity: 0,
  //       stagger: 0.15,
  //       duration: 1.2,
  //       ease: "power3.out",
  //     },
  //     "-=0.1",
  //   );

  //   // ▶ play AFTER swan timeline completes
  //   masterTl.current.add(bubbleTl);

  //   return () => {
  //     bubbleTl.kill();
  //   };
  // }, []);

  useLayoutEffect(() => {
    if (!masterTl.current || !containerRef.current) return;

    // 👇 Hard hide initially
    gsap.set(containerRef.current, {
      autoAlpha: 0,
      pointerEvents: "none",
    });

    const bubbleTl = gsap.timeline({
      paused: true,
      onStart: () => {
        gsap.set(containerRef.current, {
          pointerEvents: "auto",
        });
      },
      onComplete: () => {
        // setDropStarted(true); // 🔥 physics starts AFTER entrance
      },
    });

    // Scene fade in
    bubbleTl.to(containerRef.current, {
      autoAlpha: 1,
      duration: 0.6,
      ease: "power2.out",
    });

    // Bubbles entrance
    bubbleTl.from(
      ".float-bubble",
      {
        y: 100,
        scale: 0.7,
        opacity: 0,
        stagger: 0.18,
        duration: 1.4,
        ease: "power4.out",
      },
      "-=0.2",
    );

    // 👇 Attach EXACTLY after swan complete
    masterTl.current.add(bubbleTl, "swan-complete+=0.2");

    return () => {
      bubbleTl.kill();
    };
  }, []);

  useEffect(() => {
    // slider initial hidden state
    gsap.set(".bubble-slider", {
      xPercent: 100,
      autoAlpha: 0,
      pointerEvents: "none",
    });
  }, []);

  useEffect(() => {
    // open slider when bubble is clicked
    if (_activeId) {
      gsap.to(".bubble-slider", {
        xPercent: 0,
        autoAlpha: 1,
        duration: 0.7,
        ease: "power3.out",
        pointerEvents: "auto",
      });
    }
  }, [_activeId]);

  const handleResetScene = () => {
    gsap.to(".bubble-slider", {
      xPercent: 100,
      autoAlpha: 0,
      duration: 0.6,
      ease: "power3.in",
      pointerEvents: "none",
      onComplete: () => {
        setActiveId(null);
        setFocusTarget(null);
        setResetSignal((v) => v + 1);
      },
    });
  };

  const Lights = () => {
    return (
      <>
        {/* 🌫 Base visibility (don’t kill reflections) */}
        <ambientLight intensity={0.6} />

        {/* ☀ KEY LIGHT — main shine driver */}
        <directionalLight position={[-8, 12, 8]} intensity={4} castShadow />

        {/* ✨ RIM LIGHT — makes bubble edges glow */}
        <spotLight
          position={[0, 6, -10]}
          intensity={5}
          angle={0.35}
          penumbra={1}
          color="#cce6ff"
        />

        {/* 💎 SPECULAR SPARKS (critical for bubble feel) */}
        <pointLight position={[6, 2, 4]} intensity={1} color="#ffffff" />
        <pointLight position={[-6, -2, -4]} intensity={0.7} color="#bcdcff" />
      </>
    );
  };

  return (
    <div ref={containerRef} className="fixed inset-0 z-30">
      <div>
        <div className="fixed inset-0 w-screen h-screen bg-black">
          <Canvas
            className="w-full h-full"
            camera={{ position: [0, 4, 18], fov: 45 }}
            onPointerMissed={() => {
              (setFocused(false), setActiveId(null), setFocusTarget(null));
            }}
          >
            {/**Lights On to Objects */}
            <Lights />

            {/* 🧭 Controls (locked zoom for cinematic feel) */}

            {/* 🎥 Camera focus controller */}
            <CameraFocusController
              target={focusTarget}
              enabled={!!focusTarget}
            />

            {/* <FeatherDragDrop /> */}

            {/* 🪶 Drop your feathers here */}
            {bubbles.map((b) => (
              <FloatBubble
                key={b.id}
                id={b.id}
                position={b.position as [number, number, number]}
                radius={4.5}
                setActiveId={(id) => setActiveId(String(id))}
                setFocusTarget={(v) => setFocusTarget(v)}
                resetSignal={resetSignal} // 👈 NEW
              />
            ))}
          </Canvas>
        </div>

        {/* 👉 SLIDER (HTML UI) */}
        <div
          className="
              bubble-slider
              fixed
              top-10
              right-10
              h-[calc(100vh-5rem)]
              w-[520px]
              rounded-2xl
              bg-black/20
              backdrop-blur-2xl
              shadow-2xl
              z-30
              overflow-hidden
              pad
            "
        >
          {/* Close button */}
          <button
            className="
      absolute
      top-5
      right-5
      text-white/80
      border
      border-white/20
      rounded-full
      px-4
      py-2
      text-sm
      hover:bg-white/10
      transition
    "
            onClick={handleResetScene}
          >
            ✕ Close
          </button>

          {/* Content */}
          <div className=" text-white">
            <h2 className="text-3xl font-light tracking-wide mb-6">
              Bubble Details
            </h2>

            <div className="space-y-4 text-white/70">
              <p>
                <span className="uppercase text-xs tracking-widest text-white/40">
                  Active Bubble
                </span>
                <br />
                <span className="text-lg">{_activeId}</span>
              </p>

              <div className="h-px w-full bg-white/10 my-6" />

              <p className="leading-relaxed">
                This feather represents a unique moment in the experience. Each
                bubble opens contextual content with cinematic motion and fluid
                interaction.
              </p>
            </div>
          </div>
        </div>

        {/* <div className="gallery_slider fixed bottom-40 left-10  z-10 ">
        <button className="close border p-4" onClick={handleResetScene}>
          Close feather
        </button>
      </div> */}
      </div>
    </div>
  );
};

export default BubbleFeather_Interaction;

// const BubbleFeather_Interaction = ({ masterTl }: any) => {

//   useLayoutEffect(() => {
//     if (!masterTl?.current || !containerRef.current) return;

//     // 🎬 Bubble intro timeline
//     const bubbleTl = gsap.timeline();

//     // OPTIONAL: container reveal (quick, subtle)
//     bubbleTl.from(containerRef.current, {
//       autoAlpha: 0,
//       duration: 0.4,
//       ease: "power2.out",
//     });

//     // 🪶 Feather / bubble intro (example hooks)
//     bubbleTl.from(
//       ".float-bubble", // class inside FloatBubble
//       {
//         y: 80,
//         scale: 0.8,
//         opacity: 0,
//         stagger: 0.15,
//         duration: 1.2,
//         ease: "power3.out",
//       },
//       "-=0.1",
//     );

//     // ⛓️ ADD AFTER MENU FINISHES (SAFE & SIMPLE)
//     masterTl.current.add(bubbleTl, ">");

//     return () => {
//       bubbleTl.kill();
//     };
//   }, []);

//   return (
//     <div ref={containerRef} className="fixed inset-0 z-30">
//       <Scene />
//     </div>
//   );
// };

// useLayoutEffect(() => {
//   if (!masterTl?.current || !containerRef.current) return;

//   const bubbleTl = gsap.timeline();

//   bubbleTl.from(containerRef.current, {
//     autoAlpha: 0,
//     duration: 0.4,
//     ease: "power2.out",
//   });

//   bubbleTl.from(
//     ".float-bubble",
//     {
//       y: 80,
//       scale: 0.8,
//       opacity: 0,
//       stagger: 0.15,
//       duration: 1.2,
//       ease: "power3.out",
//     },
//     "-=0.1",
//   );

//   // 🔑 THIS is the magic line
//   masterTl.current.add(bubbleTl, ">");

//   return () => {
//     bubbleTl.kill();
//   };
// }, []);
